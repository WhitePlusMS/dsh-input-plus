 ...
 */
export async function resolveReference(root, rel, opts = {}) {
    const sepCh = opts.sepOverride ?? sep;
    const normalized = normalizeRelative(rel, sepCh);
    if (normalized === null)
        throw new UnsupportedReferenceError('OUT_OF_BOUNDS', `Rejected path: "${rel}"`);
...
export function normalizeRelative(rel, sepCh = sep) {
...
export async function indexWorkspace(opts) {
    ...
    const walk = async (dirAbs, depth) => {
        ...
        for (const entry of entries) {
            if (out.length >= opts.maxIndexEntries)
                return;
            const name = entry.name;
            if (ignored.has(name) || ignoredFiles.has(name.toLowerCase()))
                continue;
 ...
 */
export function matchScore(rel, query) {
    ...
    if (!q.includes('/')) {
        ...
        if (base.includes(q))
            return 2;
        if (lower.split('/').some((seg) => seg.startsWith(q)))
            return 3;
        if (lower.includes(q))
            return 4;
        return null;
...
export function searchCandidates(index, query, limit) {
    const q = query.toLowerCase();
