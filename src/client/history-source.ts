...
export function createHistoryStore(limit = HISTORY_LIMIT): HistoryStore {
...
export function historyFilter(query: string): string | null {
  const value = query.trim()
  const lower = value.toLowerCase()
  if (lower === 'h') return ''
  if (lower === 'history') return ''
  if (lower.startsWith('h ')) return value.slice(2).trim()
  if (lower.startsWith('history ')) return value.slice('history '.length).trim()
  return null
}

/** Build the two-line-capable visible label while keeping the full draft hidden in `hint`. */
export function historyCandidate(text: string): InputTriggerCandidate {
...
export function createHistoryInputSource(store: HistoryStore): InputTriggerSource {
  return {
...
export function installHistoryMenuStyles(): () => void {
  if (typeof document === 'undefined') return () => undefined
  if (document.getElementById(HISTORY_STYLE_ID) !== null) return () => undefined
