...
import { element, type RenderNode } from './react-runtime.js'
...
export function InputStatus(rawProps: unknown): RenderNode {
  const props = rawProps as InputStatusProps
...
export function formatInputStatus(props: InputStatusProps): string {
  const parts: string[] = []
  const references = countReferences(props.input.draft) + props.input.occurrences.length
  if (references > 0) parts.push(`引用 ${references}`)
  if (props.input.imageIds.length > 0) parts.push(`附件 ${props.input.imageIds.length}`)
  if (props.session.queue.length > 0) parts.push(`排队 ${props.session.queue.length}`)
  if (props.session.running) parts.push('运行中')
  if (props.input.phase !== 'plain') parts.push('输入处理中')
  if (props.session.removed) parts.push('会话已关闭')
  return parts.join(' · ')
}

function countReferences(draft: string): number {
  const re = /(?:^|[\s(])@([^\s@()]+)/g
  let count = 0
  while (re.exec(draft) !== null) count += 1
  return count
}
