...
export function parseGitStatus(stdout: string): ReadonlySet<string> {
...
export async function readGitChangedFiles(root: string): Promise<ReadonlySet<string>> {
  try {
    const result = await execFileAsync(
      'git',
      ['-C', root, 'status', '--porcelain=v1', '-z', '--untracked-files=all', '--', '.'],
      { encoding: 'utf8', maxBuffer: MAX_STATUS_BYTES, windowsHide: true },
    )
